...
class BasicStrategy(bt.Strategy):
    ...
    def __init__(self):
        ...
        self.max_loss_percent = 0.02 # max risk percent from protfolio value per trade
        #self.symbol_params = symbol_setup[self.p.symbol]
        self.symbol_parameter = SymbolParameter(self.p.symbol, self.broker.get_cash(), self.max_loss_percent)
        ...
        if not self.position and self.stage == self.Start:
            if self.stddev * self.p.squeeze_bb_multiplier > self.atr * self.p.squeeze_kc_multiplier:
                ...
                if (self.macd_ema[0] > self.macd_ema[-1] and self.macd.histo[0] > self.macd.histo[-1]) and (
                    ...
                    log.debug('{} size: {:.0f}'.format(self.data.datetime.datetime(0), self.size))
